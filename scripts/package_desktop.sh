...
# Usage: ./scripts/package_desktop.sh [VERSION] [lite|advisor]
#
...
if [[ "$EDITION" != "lite" && "$EDITION" != "advisor" ]]; then
  echo "Edition must be lite or advisor (got: $EDITION)" >&2
  exit 1
fi
if [[ -z "$VERSION" ]]; then
...
if [[ "$EDITION" == "advisor" ]]; then
  APP_NAME="Nakshatra Chakram Advisor"
fi
BUNDLE_ID="com.avinashpeyyety.nakshatra-chakram"
DIST="$ROOT/dist"
STAGE="$DIST/stage-${EDITION}"
rm -rf "$STAGE"
mkdir -p "$STAGE"

echo "==> Packaging ${APP_NAME} v$VERSION ($EDITION)"
...
cp "$ROOT/.env.example" "$APP_PAYLOAD/" 2>/dev/null || true
printf '%s\n' "$EDITION" > "$APP_PAYLOAD/agent/edition.txt"
...
export NAKSHATRA_EDITION=${EDITION}

APP_ROOT="\$(cd "\$(dirname "\$0")/../Resources/app" && pwd)"
SUPPORT_DIR="\${HOME}/Library/Application Support/Nakshatra Chakram"
if [[ "${EDITION}" == "advisor" ]]; then
  SUPPORT_DIR="\${HOME}/Library/Application Support/Nakshatra Chakram Advisor"
fi
...
mkdir -p "\$SUPPORT_DIR"
exec >>"\$LOG_FILE" 2>&1
echo "=== Launch \$(date) edition=${EDITION} ==="
...
if [[ -z "\$PY" ]]; then
  osascript -e 'display alert "Python 3.10+ required" message "Install Python 3.11 or 3.12 from https://www.python.org/downloads/, then open Nakshatra Chakram again."' 2>/dev/null \\
...
for _ in \$(seq 1 90); do
  ...
  if ! kill -0 "\$PID" 2>/dev/null; then
    osascript -e 'display alert "Nakshatra Chakram failed to start" message "See launcher.log in Application Support."' 2>/dev/null || true
    exit 1
  fi
  sleep 2
done
...
README
    ...
    DMG_OUT="$DIST/nakshatra-chakram-${VERSION}-${EDITION}-macos.dmg"
    rm -f "$DMG_OUT"
    hdiutil create \
      -volname "Nakshatra ${EDITION} ${VERSION}" \
      -srcfolder "$DMG_STAGE" \
      -ov \
      -format UDZO \
      "$DMG_OUT" >/dev/null
    ...
    if [[ "$EDITION" == "lite" ]]; then
      cp "$DMG_OUT" "$DIST/nakshatra-chakram-${VERSION}-macos.dmg"
    fi
    ;;

  MINGW*|MSYS*|CYGWIN*)
    WIN_STAGE="$STAGE/nakshatra-chakram-${VERSION}-${EDITION}"
    cp -R "$APP_PAYLOAD/" "$WIN_STAGE/"
    cat > "$WIN_STAGE/run-windows.bat" << RUNWIN
@echo off
cd /d "%~dp0"
set NAKSHATRA_ADMIN=0
set NAKSHATRA_EDITION=${EDITION}
...
RUNWIN
    ZIP_OUT="$DIST/nakshatra-chakram-${VERSION}-${EDITION}-windows.zip"
    (cd "$STAGE" && zip -rq "$ZIP_OUT" "nakshatra-chakram-${VERSION}-${EDITION}")
    echo "==> Created $ZIP_OUT"
    ;;

  *)
    ZIP_OUT="$DIST/nakshatra-chakram-${VERSION}-${EDITION}.zip"
    FALLBACK="$STAGE/nakshatra-chakram-${VERSION}-${EDITION}"
...
echo ""
echo "Next: package other edition and ./scripts/publish_github_release.sh $VERSION"
